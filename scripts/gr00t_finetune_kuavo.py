...
class ArgsConfig:
    ...
    data_config: str = "kuavo_full_upper_body"
    """
...
if __name__ == "__main__":
    ...
    # 父进程：根据 gpu_ids 决定使用哪些卡，并启动 torchrun
    selected_gpu_ids = config.gpu_ids if config.gpu_ids is not None else list(range(config.num_gpus))
    assert len(selected_gpu_ids) > 0, "Please specify at least one GPU via --gpu-ids or set --num-gpus > 0"

    nproc = len(selected_gpu_ids)
    print(f"Using GPUs: {selected_gpu_ids} (nproc_per_node={nproc})")
